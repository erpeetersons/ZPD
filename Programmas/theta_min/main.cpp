...
void createOutput(d phi) //Determines all minimums of function E in respect to H.
{
    ofstream out;
    out.open("output.txt");
    d H = -1;
    ll nH = 10000; //Number of sections that H will be divided into.
    ...
    for(ll i = 0; i < nH; i ++) //Goes trough all H values and finds minimums of function E.
    {
        ...
        points[i][1] = 5;
        d theta = 0;
        ll nT = 10000; //Number of sections that theta will be divided into.
    ...
    for(ll i = nH - 1; i >= 0; i --) //Outputs upper part of function.
    {
        out << H << ' ' << points[i][1] << '\n';
        H -= intervalH;
    }
    for(ll i = 0; i < nH; i ++) //Outputs lower part of function.
    {
        out << H << ' ' << points[i][0] << '\n';
        H += intervalH;
    }
...
int main()
{
    ...
    createOutput(phi); //Calculates points for output function.
